...
class Strings:
    ...
    RU = {
        ...
        # Tooltips
        "toggle_folding": "Cворачивание кода",
...
def get_safe_monospace_font(preferred_font: str, size: int) -> QFont:
    ...
    return QFont(MONOSPACE_FONT_NAME, size)
...
""".split('\n')
    ...
    def _setup_ui(self):
        ...
        self.code_area.setMinimumHeight(320)
        ...
        while i < n:
            ...
            if line[i].isalpha() or line[i] == '_' or line[i] == '#':
                ...
                if word == "namespace" or word == "struct":
                    ...
                    elif word in lexer.define_macros:
                        color = colors.get("define_macro", colors["fg"])
    ...
    def paintEvent(self, event):
        ...
        if self.tokenized_lines:
            ...
            font = QFont(MONOSPACE_FONT_NAME, 10)
    ...
    def update_theme(self):
        ...
        self.setStyleSheet(f"""
            ...
            QTreeView::item:hover {{
                background-color: {colors['tree_hover_bg'].name()};
                color: {colors['tree_hover_fg'].name()};
            }}
            QTreeView::item:selected {{
                background-color: {colors['tree_selection_bg'].name()};
                color: {colors['tree_selection_fg'].name()};
            }}
            ...
            QTreeView QLineEdit {{
                background-color: {colors['tree_edit_bg'].name()};
                color: {colors['tree_edit_fg'].name()};
                border: none;
                outline: none;
                padding: 2px;
                margin-top: -2px;
...
class ErrorOverviewPanel(QWidget):
    ...
    def set_all_errors(self, file_errors_map: Dict[str, List]):
        ...
        # Итерируемся по всем файлам и их ошибкам
        for file_path, errors in file_errors_map.items():
...
class TwistLangLexer(QsciLexerCustom):
    ...
    def setup_styles(self):
        ...
        # Специальные шрифты для отдельных стилей
        ...
        # Для модификаторов - курсив с подчеркиванием
        modifier_font = QFont(safe_font)
        modifier_font.setItalic(True)
    ...
    def styleText(self, start: int, end: int):
        ...
        # Собираем все define-макросы из ВСЕГО текста
        ...
        while pos < end:
            ...
            if ch in ('"', "'"):
                ...
                # Подсвечиваем открывающую кавычку
                ...
                while j < end:
                    ...
                    if c == '\\':
                        ...
                        j += l
                        if j < end:
                            _, next_l = self._get_char_at(text_bytes, j, total_bytes)
                            esc_len += next_l
                            j += next_l
            ...
            if ch.isdigit() or (ch == '.' and pos + ch_len < end and
                ...
                while j < end:
                    c, l = self._get_char_at(text_bytes, j, total_bytes)
                    if not (c.isdigit() or (c == '.' and not has_dot)):
                        break
                    if c == '.':
                        has_dot = True
                    j += l
...
class CustomScintilla(QsciScintilla):
    ...
    def __init__(self, parent=None):
        ...
        self.ls_debounce_timer = QTimer(self)
    ...
    def _setup_python_lexer(self, lexer: QsciLexerPython):
        ...
        font = get_safe_monospace_font(MONOSPACE_FONT_NAME, font_size)
    ...
    def _request_ls_check(self):
        if self.main_window and self.filename:
            self.main_window.request_language_server_check(self.filename)
    ...
    def _paint_error_for_line(self, painter: QPainter, line: int, colors: dict, margin_width: int):
        try:
            ...
            font = QFont(MONOSPACE_FONT_NAME, 9)
            if self.main_window:
...
class CodeSnapDialog(QWidget):
    ...
    def _calculate_size(self):
        font = QFont(MONOSPACE_FONT_NAME, int(12 * self.scale_factor))
    ...
    def _draw_code(self, painter: QPainter, colors: dict):
        ...
        if visible_lines < len(self.lines):
            ...
            font = QFont(MONOSPACE_FONT_NAME, int(10 * self.scale_factor))
            font.setItalic(True)
            painter.setFont(font)
            metrics = QFontMetrics(font)
...
class TwistLangEditor(FramelessMainWindow):
    ...
    def _rebuild_menus(self):
        ...
        settings_menu.addSeparator()
        self.folding_action = QAction(Strings.get("toggle_folding"), self)
        self.folding_action.setCheckable(True)
        self.folding_action.setChecked(self.code_folding_enabled)
        self.folding_action.setFont(consolas_font)
        self.folding_action.triggered.connect(self._toggle_folding_handler)
        settings_menu.addAction(self.folding_action)
        settings_menu.addSeparator()
    ...
    def request_language_server_check(self, file_path: str):
    ...
    def _update_checkmarks_after_rebuild(self):
        ...
        if hasattr(self, 'folding_action'):
            if self.code_folding_enabled:
                self.folding_action.setChecked(True)
                self.folding_action.setIcon(self.check_icon)
            else:
                self.folding_action.setChecked(False)
                self.folding_action.setIcon(QIcon())
    ...
    def _update_error_overview(self):
        file_errors_map = {}
        # Собираем ошибки со всех открытых вкладок
        for i in range(self.tab_widget.count()):
            editor = self.tab_widget.widget(i)
    ...
    def _toggle_folding_handler(self, enabled: bool):
        self.code_folding_enabled = enabled
        for i in range(self.tab_widget.count()):
            editor = self.tab_widget.widget(i)
            if isinstance(editor, CustomScintilla):
        ...
        if hasattr(self, 'folding_action'):
            if enabled:
                colors = THEMES[self.current_theme]["colors"]
                self.folding_action.setIcon(self._create_check_icon(colors["fg"]))
            else:
                self.folding_action.setIcon(QIcon())
    ...
    def _add_editor_tab(self, editor: CustomScintilla, filename: Optional[str], title: str):
        ...
        editor.set_folding_visible(self.code_folding_enabled)
        if self.code_folding_enabled:
            self._apply_fold_marker_colors(editor)
        ...
        try:
            with open(error_file_path, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    if match:
                        file_path = match.group(1)  # Сохраняем путь файла
                        ...
                        # Добавляем file_path первым элементом кортежа
                        errors.append((
                            file_path,       # 0: путь к файлу ошибки
                            line_num - 1,    # 1: строка
                            pos - 1,         # 2: начальная колонка
                            (pos - 1) + length,  # 3: конечная колонка
                            message,         # 4: сообщение
                            t                # 5: тип ошибки
                        ))
        except Exception as e:
    ...
    def _filter_errors_by_file(self, errors: List[tuple], current_file_path: str) -> List[tuple]:
        """Оставляет только ошибки, принадлежащие указанному файлу."""
        filtered = []
        # Нормализуем путь текущего файла
        norm_current = os.path.normpath(current_file_path).replace('\\', '/').lower()

        for error in errors:
            error_file_path = error[0]  # Путь к файлу с ошибкой
            # Нормализуем путь из ошибки
            norm_error = os.path.normpath(error_file_path).replace('\\', '/').lower()

            # Сравниваем: либо полное совпадение, либо по имени файла
            if norm_error == norm_current or os.path.basename(norm_error) == os.path.basename(norm_current):
                # Убираем путь из кортежа, оставляя только старый формат для set_errors
                filtered.append(error[1:])  # (line, start_col, end_col, message, type)
