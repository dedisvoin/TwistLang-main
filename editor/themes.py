from PyQt6.QtGui import QColor

THEMES = {

    # Light themes (editor lighter than panels — standard behavior)
    "Lumen Classic": {
        "type": "light",
        "colors": {
            "bg": QColor("#eff1f5"),
            "fg": QColor("#4c4f69"),
            "title_bg": QColor("#d0d5de"),
            "title_bg_darker": QColor("#c9cdd8"),
            "title_fg": QColor("#4c4f69"),
            "title_border": QColor("#bcc0cc"),
            "keyword": QColor("#859900"),
            "type": QColor("#b58900"),
            "comment": QColor("#93a1a1"),
            "string": QColor("#2aa198"),
            "number": QColor("#d33682"),
            "operator": QColor("#657b83"),
            "function": QColor("#155e93"),
            "modifier": QColor("#6c71c4"),
            "directive": QColor("#cb4b16"),
            "literal": QColor("#dc322f"),
            "namespace": QColor("#268bd2"),
            "special": QColor("#d33682"),
            "object": QColor("#2a9781"),
            "define_macro": QColor("#bcc0cc"),
            "dunder_method": QColor("#6c71c4"),  # Фиолетово-синий, хорошо сочетается с модификаторами
            "caret": QColor("#1e1e2e"),
            "caret_line": QColor("#e6e9ef"),
            "margin_bg": QColor("#dce0e8"),
            "margin_fg": QColor("#6c6f85"),
            "selection_bg": QColor("#acb0be"),
            "selection_fg": QColor("#1e1e2e"),
            "brace_bg": QColor("#8839ef55"),
            "brace_fg": QColor("#8839ef"),
            "error": QColor("#f35815"),
            "warning": QColor("#ffc413"),
            "echo": QColor("#9ca0b0"),
            "autosave_on": QColor("#a6e3a1"),
            "autosave_off": QColor("#f38ba8"),
            "status_bg": QColor("#2d2d3a"),
            "status_border": QColor("#4c4f69"),
            "tree_hover_bg": QColor("#e6e9ef"),
            "tree_hover_fg": QColor("#4c4f69"),
            "tree_selection_bg": QColor("#dce0e8"),
            "tree_selection_fg": QColor("#4c4f69"),
            "tree_edit_bg": QColor("#eff1f5"),
            "tree_edit_fg": QColor("#4c4f69"),
        }
    },

    # Тема Monokai Pro (светлая)
"Monokai Pro Light": {
    "type": "light",
    "colors": {
        "bg": QColor("#fcfcfa"),
        "fg": QColor("#2d2a2e"),
        "title_bg": QColor("#e6e5e2"),
        "title_bg_darker": QColor("#d9d7d3"),
        "title_fg": QColor("#2d2a2e"),
        "title_border": QColor("#ceccc8"),
        "keyword": QColor("#f92672"),
        "type": QColor("#0088cc"),
        "comment": QColor("#8c8a86"),
        "string": QColor("#a0b432"),
        "number": QColor("#ae81ff"),
        "operator": QColor("#2d2a2e"),
        "function": QColor("#758f22"),
        "modifier": QColor("#fd971f"),
        "directive": QColor("#f92672"),
        "literal": QColor("#ae81ff"),
        "namespace": QColor("#0088cc"),
        "special": QColor("#f92672"),
        "object": QColor("#758f22"),
        "define_macro": QColor("#b0aeaa"),
        "dunder_method": QColor("#758f22"),
        "caret": QColor("#2d2a2e"),
        "caret_line": QColor("#efedea"),
        "margin_bg": QColor("#f0eeeb"),
        "margin_fg": QColor("#b0aeaa"),
        "selection_bg": QColor("#d6d4d0"),
        "selection_fg": QColor("#2d2a2e"),
        "brace_bg": QColor("#f9267255"),
        "brace_fg": QColor("#f92672"),
        "error": QColor("#f44747"),
        "warning": QColor("#fd971f"),
        "echo": QColor("#b0aeaa"),
        "autosave_on": QColor("#758f22"),
        "autosave_off": QColor("#f44747"),
        "status_bg": QColor("#e6e5e2"),
        "status_border": QColor("#ceccc8"),
        "tree_hover_bg": QColor("#efedea"),
        "tree_hover_fg": QColor("#2d2a2e"),
        "tree_selection_bg": QColor("#e6e5e2"),
        "tree_selection_fg": QColor("#2d2a2e"),
        "tree_edit_bg": QColor("#fcfcfa"),
        "tree_edit_fg": QColor("#2d2a2e"),
    }
},


    "Pure Light": {
        "type": "light",
        "colors": {
            "bg": QColor("#ffffff"),
            "fg": QColor("#202020"),
            "title_bg": QColor("#e3e0e0"),
            "title_bg_darker": QColor("#e8e8e8"),
            "title_fg": QColor("#202020"),
            "title_border": QColor("#d0d0d0"),
            "keyword": QColor("#0000cc"),
            "type": QColor("#007a7a"),
            "comment": QColor("#6a6a6a"),
            "string": QColor("#a31515"),
            "number": QColor("#09885a"),
            "operator": QColor("#202020"),
            "function": QColor("#795e26"),
            "modifier": QColor("#6c71c4"),
            "directive": QColor("#ca4b16"),
            "literal": QColor("#b80000"),
            "namespace": QColor("#267f99"),
            "special": QColor("#d33682"),
            "object": QColor("#2a9781"),
            "define_macro": QColor("#c0c0c0"),
            "dunder_method": QColor("#8b5cf6"),  # Яркий фиолетовый для контраста на белом фоне
            "caret": QColor("#000000"),
            "caret_line": QColor("#f0f0f0"),
            "margin_bg": QColor("#f8f8f8"),
            "margin_fg": QColor("#808080"),
            "selection_bg": QColor("#cce0ff"),
            "selection_fg": QColor("#000000"),
            "brace_bg": QColor("#0078d755"),
            "brace_fg": QColor("#0078d7"),
            "error": QColor("#d73a49"),
            "warning": QColor("#e36209"),
            "echo": QColor("#a0a0a0"),
            "autosave_on": QColor("#28a745"),
            "autosave_off": QColor("#d73a49"),
            "status_bg": QColor("#2d2d30"),
            "status_border": QColor("#d0d0d0"),
            "tree_hover_bg": QColor("#f0f0f0"),
            "tree_hover_fg": QColor("#202020"),
            "tree_selection_bg": QColor("#e8e8e8"),
            "tree_selection_fg": QColor("#202020"),
            "tree_edit_bg": QColor("#ffffff"),
            "tree_edit_fg": QColor("#202020"),
        }
    },

    "Catppuccin Latte": {
        "type": "light",
        "colors": {
            "bg": QColor("#eff1f5"),
            "fg": QColor("#4c4f69"),
            "title_bg": QColor("#d3d6dd"),
            "title_bg_darker": QColor("#dce0e8"),
            "title_fg": QColor("#4c4f69"),
            "title_border": QColor("#ccd0da"),
            "keyword": QColor("#8839ef"),
            "type": QColor("#179299"),
            "comment": QColor("#acb0be"),
            "string": QColor("#40a02b"),
            "number": QColor("#fe640b"),
            "operator": QColor("#5c5f77"),
            "function": QColor("#df8e1d"),
            "modifier": QColor("#8839ef"),
            "directive": QColor("#d20f39"),
            "literal": QColor("#fe640b"),
            "namespace": QColor("#179299"),
            "special": QColor("#ea76cb"),
            "object": QColor("#209fb5"),
            "define_macro": QColor("#bcc0cc"),
            "dunder_method": QColor("#8839ef"),  # Тот же цвет, что у ключевых слов Catppuccin
            "caret": QColor("#4c4f69"),
            "caret_line": QColor("#e6e9ef"),
            "margin_bg": QColor("#e6e9ef"),
            "margin_fg": QColor("#acb0be"),
            "selection_bg": QColor("#ccd0da"),
            "selection_fg": QColor("#4c4f69"),
            "brace_bg": QColor("#8839ef55"),
            "brace_fg": QColor("#8839ef"),
            "error": QColor("#d20f39"),
            "warning": QColor("#fe640b"),
            "echo": QColor("#9ca0b0"),
            "autosave_on": QColor("#40a02b"),
            "autosave_off": QColor("#d20f39"),
            "status_bg": QColor("#2d2d3a"),
            "status_border": QColor("#ccd0da"),
            "tree_hover_bg": QColor("#e6e9ef"),
            "tree_hover_fg": QColor("#4c4f69"),
            "tree_selection_bg": QColor("#dce0e8"),
            "tree_selection_fg": QColor("#4c4f69"),
            "tree_edit_bg": QColor("#eff1f5"),
            "tree_edit_fg": QColor("#4c4f69"),
        }
    },

    "Kanagawa Light": {
        "type": "light",
        "colors": {
            "bg": QColor("#f8f4ed"),
            "fg": QColor("#3c3c3c"),
            "title_bg": QColor("#dbd6cd"),
            "title_bg_darker": QColor("#ded8ce"),
            "title_fg": QColor("#3c3c3c"),
            "title_border": QColor("#d0cabc"),
            "keyword": QColor("#ab8b5c"),
            "type": QColor("#5c7eb5"),
            "comment": QColor("#9e9b93"),
            "string": QColor("#6f8f3c"),
            "number": QColor("#c46a6a"),
            "operator": QColor("#9e9b93"),
            "function": QColor("#5f8b9c"),
            "modifier": QColor("#7e6b9c"),
            "directive": QColor("#e48c4c"),
            "literal": QColor("#e48c4c"),
            "namespace": QColor("#5c7eb5"),
            "special": QColor("#d5c254"),
            "object": QColor("#6f8f3c"),
            "define_macro": QColor("#c4bfb2"),
            "dunder_method": QColor("#7e6b9c"),  # Тот же цвет, что у модификаторов Kanagawa
            "caret": QColor("#3c3c3c"),
            "caret_line": QColor("#e9e3d9"),
            "margin_bg": QColor("#ede6dc"),
            "margin_fg": QColor("#9e9b93"),
            "selection_bg": QColor("#d6cfc2"),
            "selection_fg": QColor("#3c3c3c"),
            "brace_bg": QColor("#ab8b5c55"),
            "brace_fg": QColor("#ab8b5c"),
            "error": QColor("#c46a6a"),
            "warning": QColor("#e48c4c"),
            "echo": QColor("#aaa49a"),
            "autosave_on": QColor("#6f8f3c"),
            "autosave_off": QColor("#c46a6a"),
            "status_bg": QColor("#2f2f2f"),
            "status_border": QColor("#b8b2a4"),
            "tree_hover_bg": QColor("#ede6dc"),
            "tree_hover_fg": QColor("#3c3c3c"),
            "tree_selection_bg": QColor("#e9e3d9"),
            "tree_selection_fg": QColor("#3c3c3c"),
            "tree_edit_bg": QColor("#f8f4ed"),
            "tree_edit_fg": QColor("#3c3c3c"),
        }
    },

    "Solarized Light": {
        "type": "light",
        "colors": {
            "bg": QColor("#fdf6e3"),
            "fg": QColor("#657b83"),
            "title_bg": QColor("#ded8c6"),
            "title_bg_darker": QColor("#ddd6c3"),
            "title_fg": QColor("#586e75"),
            "title_border": QColor("#93a1a1"),
            "keyword": QColor("#859900"),
            "type": QColor("#b58900"),
            "comment": QColor("#93a1a1"),
            "string": QColor("#2aa198"),
            "number": QColor("#d33682"),
            "operator": QColor("#657b83"),
            "function": QColor("#268bd2"),
            "modifier": QColor("#6c71c4"),
            "directive": QColor("#cb4b16"),
            "literal": QColor("#dc322f"),
            "namespace": QColor("#268bd2"),
            "special": QColor("#d33682"),
            "object": QColor("#b58900"),
            "define_macro": QColor("#cdc8b5"),
            "dunder_method": QColor("#6c71c4"),  # Лавандовый, сочетается с Solarized
            "caret": QColor("#002b36"),
            "caret_line": QColor("#eee8d5"),
            "margin_bg": QColor("#eee8d5"),
            "margin_fg": QColor("#93a1a1"),
            "selection_bg": QColor("#93a1a1"),
            "selection_fg": QColor("#002b36"),
            "brace_bg": QColor("#85990055"),
            "brace_fg": QColor("#859900"),
            "error": QColor("#dc322f"),
            "warning": QColor("#b58900"),
            "echo": QColor("#93a1a1"),
            "autosave_on": QColor("#2aa198"),
            "autosave_off": QColor("#dc322f"),
            "status_bg": QColor("#073642"),
            "status_border": QColor("#586e75"),
            "tree_hover_bg": QColor("#eee8d5"),
            "tree_hover_fg": QColor("#657b83"),
            "tree_selection_bg": QColor("#ddd6c3"),
            "tree_selection_fg": QColor("#657b83"),
            "tree_edit_bg": QColor("#fdf6e3"),
            "tree_edit_fg": QColor("#657b83"),
        }
    },

    "Visual Studio Light": {
        "type": "light",
        "colors": {
            "bg": QColor("#ffffff"),
            "fg": QColor("#000000"),
            "title_bg": QColor("#d4d2d2"),
            "title_bg_darker": QColor("#dddddd"),
            "title_fg": QColor("#000000"),
            "title_border": QColor("#cccccc"),
            "keyword": QColor("#0000ff"),
            "type": QColor("#2b91af"),
            "comment": QColor("#008000"),
            "string": QColor("#a31515"),
            "number": QColor("#09885a"),
            "operator": QColor("#000000"),
            "function": QColor("#795e26"),
            "modifier": QColor("#0000ff"),
            "directive": QColor("#008000"),
            "literal": QColor("#a31515"),
            "namespace": QColor("#267f99"),
            "special": QColor("#ff0000"),
            "object": QColor("#2b91af"),
            "define_macro": QColor("#c0c0c0"),
            "dunder_method": QColor("#6c71c4"),  # Фиолетовый для контраста с VS стилем
            "caret": QColor("#000000"),
            "caret_line": QColor("#f0f0f0"),
            "margin_bg": QColor("#f0f0f0"),
            "margin_fg": QColor("#606060"),
            "selection_bg": QColor("#fff8d3"),
            "selection_fg": QColor("#000000"),
            "brace_bg": QColor("#0078d755"),
            "brace_fg": QColor("#0078d7"),
            "error": QColor("#ff0000"),
            "warning": QColor("#ff8c00"),
            "echo": QColor("#6a6a6a"),
            "autosave_on": QColor("#107c10"),
            "autosave_off": QColor("#d73a49"),
            "status_bg": QColor("#2d2d2d"),
            "status_border": QColor("#cccccc"),
            "tree_hover_bg": QColor("#f0f0f0"),
            "tree_hover_fg": QColor("#000000"),
            "tree_selection_bg": QColor("#e8e8e8"),
            "tree_selection_fg": QColor("#000000"),
            "tree_edit_bg": QColor("#ffffff"),
            "tree_edit_fg": QColor("#000000"),
        }
    },

    # ===== DARK THEMES (editor darker than panels) =====
    "Moonlight": {
        "type": "dark",
        "colors": {
            "bg": QColor("#15222e"),
            "fg": QColor("#d9e0ee"),
            "title_bg": QColor("#2a3a4a"),
            "title_bg_darker": QColor("#22303e"),
            "title_fg": QColor("#d9e0ee"),
            "title_border": QColor("#3a4a5a"),
            "keyword": QColor("#ff8c42"),
            "type": QColor("#70c9f0"),
            "comment": QColor("#5a6e7a"),
            "string": QColor("#b3d46b"),
            "number": QColor("#ffb86c"),
            "operator": QColor("#7b8c74"),
            "function": QColor("#6b8eff"),
            "modifier": QColor("#ffcd42"),
            "directive": QColor("#ffb86c"),
            "literal": QColor("#b3d46b"),
            "namespace": QColor("#70c9f0"),
            "special": QColor("#ff6b6b"),
            "object": QColor("#70c9f0"),
            "define_macro": QColor("#4a5a6a"),
            "dunder_method": QColor("#cba6f7"),  # Лавандовый, хорошо на темном фоне
            "caret": QColor("#d9e0ee"),
            "caret_line": QColor("#1e2a3a"),
            "margin_bg": QColor("#1e2a3a"),
            "margin_fg": QColor("#5a6e7a"),
            "selection_bg": QColor("#3a4a5a"),
            "selection_fg": QColor("#d9e0ee"),
            "brace_bg": QColor("#ff8c4255"),
            "brace_fg": QColor("#ff8c42"),
            "error": QColor("#ff6b6b"),
            "warning": QColor("#ffb86c"),
            "echo": QColor("#8a9ba8"),
            "autosave_on": QColor("#b3d46b"),
            "autosave_off": QColor("#ff6b6b"),
            "status_bg": QColor("#0d1117"),
            "status_border": QColor("#2a3a4a"),
            "tree_hover_bg": QColor("#22303e"),
            "tree_hover_fg": QColor("#d9e0ee"),
            "tree_selection_bg": QColor("#2a3a4a"),
            "tree_selection_fg": QColor("#d9e0ee"),
            "tree_edit_bg": QColor("#15222e"),
            "tree_edit_fg": QColor("#d9e0ee"),
        }
    },

    "Catppuccin Mocha": {
        "type": "dark",
        "colors": {
            "bg": QColor("#181825"),
            "fg": QColor("#cdd6f4"),
            "title_bg": QColor("#313244"),
            "title_bg_darker": QColor("#29293b"),
            "title_fg": QColor("#cdd6f4"),
            "title_border": QColor("#45475a"),
            "keyword": QColor("#cba6f7"),
            "type": QColor("#96ebd0"),
            "comment": QColor("#6c7086"),
            "string": QColor("#a6e3a1"),
            "number": QColor("#fab387"),
            "operator": QColor("#9399b2"),
            "function": QColor("#f9e2af"),
            "modifier": QColor("#a6b0f7"),
            "directive": QColor("#f38ba8"),
            "literal": QColor("#fab387"),
            "namespace": QColor("#89b4fa"),
            "special": QColor("#f5c2e7"),
            "object": QColor("#94e2d5"),
            "define_macro": QColor("#4a4a5a"),
            "dunder_method": QColor("#cba6f7"),  # Тот же цвет, что у ключевых слов Catppuccin
            "caret": QColor("#cdd6f4"),
            "caret_line": QColor("#29293b"),
            "margin_bg": QColor("#1e1e2e"),
            "margin_fg": QColor("#6c7086"),
            "selection_bg": QColor("#585b70"),
            "selection_fg": QColor("#cdd6f4"),
            "brace_bg": QColor("#cba6f755"),
            "brace_fg": QColor("#cba6f7"),
            "error": QColor("#f38ba8"),
            "warning": QColor("#fab387"),
            "echo": QColor("#a6adc8"),
            "autosave_on": QColor("#a6e3a1"),
            "autosave_off": QColor("#f38ba8"),
            "status_bg": QColor("#11111b"),
            "status_border": QColor("#313244"),
            "tree_hover_bg": QColor("#29293b"),
            "tree_hover_fg": QColor("#cdd6f4"),
            "tree_selection_bg": QColor("#313244"),
            "tree_selection_fg": QColor("#cdd6f4"),
            "tree_edit_bg": QColor("#181825"),
            "tree_edit_fg": QColor("#cdd6f4"),
        }
    },

    "Lumen Dark": {
        "type": "dark",
        "colors": {
            "bg": QColor("#181825"),
            "fg": QColor("#cdd6f4"),
            "title_bg": QColor("#313244"),
            "title_bg_darker": QColor("#29293b"),
            "title_fg": QColor("#cdd6f4"),
            "title_border": QColor("#45475a"),
            "keyword": QColor("#f5c2e7"),
            "type": QColor("#cba6f7"),
            "comment": QColor("#6c7086"),
            "string": QColor("#a6e3a1"),
            "number": QColor("#fab387"),
            "operator": QColor("#6c7086"),
            "function": QColor("#89b4fa"),
            "modifier": QColor("#b4befe"),
            "directive": QColor("#f5e0dc"),
            "literal": QColor("#f9e2af"),
            "namespace": QColor("#94e2d5"),
            "special": QColor("#f2b5b5"),
            "object": QColor("#f2cdcd"),
            "define_macro": QColor("#4a4a5a"),
            "dunder_method": QColor("#b4befe"),  # Светло-фиолетовый из палитры Lumen
            "caret": QColor("#ffffff"),
            "caret_line": QColor("#29293b"),
            "margin_bg": QColor("#1e1e2e"),
            "margin_fg": QColor("#6c7086"),
            "selection_bg": QColor("#585b70"),
            "selection_fg": QColor("#ffffff"),
            "brace_bg": QColor("#f5c2e755"),
            "brace_fg": QColor("#f5c2e7"),
            "error": QColor("#f35815"),
            "warning": QColor("#ffc413"),
            "echo": QColor("#a6adc8"),
            "autosave_on": QColor("#a6e3a1"),
            "autosave_off": QColor("#f38ba8"),
            "status_bg": QColor("#11111b"),
            "status_border": QColor("#313244"),
            "tree_hover_bg": QColor("#29293b"),
            "tree_hover_fg": QColor("#cdd6f4"),
            "tree_selection_bg": QColor("#313244"),
            "tree_selection_fg": QColor("#cdd6f4"),
            "tree_edit_bg": QColor("#181825"),
            "tree_edit_fg": QColor("#cdd6f4"),
        }
    },

    "Kanagawa Dark": {
        "type": "dark",
        "colors": {
            "bg": QColor("#16161d"),
            "fg": QColor("#dcd7ba"),
            "title_bg": QColor("#2a2a37"),
            "title_bg_darker": QColor("#23232e"),
            "title_fg": QColor("#dcd7ba"),
            "title_border": QColor("#363646"),
            "keyword": QColor("#c0a36e"),
            "type": QColor("#7e9cd8"),
            "comment": QColor("#727169"),
            "string": QColor("#98bb6c"),
            "number": QColor("#e46876"),
            "operator": QColor("#727169"),
            "function": QColor("#7fb4ca"),
            "modifier": QColor("#957fb8"),
            "directive": QColor("#ffa066"),
            "literal": QColor("#ffa066"),
            "namespace": QColor("#7e9cd8"),
            "special": QColor("#e46876"),
            "object": QColor("#98bb6c"),
            "define_macro": QColor("#4b4b58"),
            "dunder_method": QColor("#957fb8"),  # Фиолетовый из палитры Kanagawa
            "caret": QColor("#dcd7ba"),
            "caret_line": QColor("#23232e"),
            "margin_bg": QColor("#1f1f28"),
            "margin_fg": QColor("#727169"),
            "selection_bg": QColor("#2e2e3a"),
            "selection_fg": QColor("#dcd7ba"),
            "brace_bg": QColor("#c0a36e55"),
            "brace_fg": QColor("#c0a36e"),
            "error": QColor("#e46876"),
            "warning": QColor("#ffa066"),
            "echo": QColor("#9e9b93"),
            "autosave_on": QColor("#98bb6c"),
            "autosave_off": QColor("#e46876"),
            "status_bg": QColor("#0e0e13"),
            "status_border": QColor("#2a2a37"),
            "tree_hover_bg": QColor("#23232e"),
            "tree_hover_fg": QColor("#dcd7ba"),
            "tree_selection_bg": QColor("#2a2a37"),
            "tree_selection_fg": QColor("#dcd7ba"),
            "tree_edit_bg": QColor("#16161d"),
            "tree_edit_fg": QColor("#dcd7ba"),
        }
    },

    "Everforest Dark": {
        "type": "dark",
        "colors": {
            "bg": QColor("#26323f"),
            "fg": QColor("#d3c6aa"),
            "title_bg": QColor("#3a4a5a"),
            "title_bg_darker": QColor("#2f3f4f"),
            "title_fg": QColor("#d3c6aa"),
            "title_border": QColor("#4a5a6a"),
            "keyword": QColor("#e6986c"),
            "type": QColor("#7fbbb3"),
            "comment": QColor("#859289"),
            "string": QColor("#a7c080"),
            "number": QColor("#e67e80"),
            "operator": QColor("#e6986c"),
            "function": QColor("#dbbc7f"),
            "modifier": QColor("#e6986c"),
            "directive": QColor("#e67e80"),
            "literal": QColor("#e67e80"),
            "namespace": QColor("#7fbbb3"),
            "special": QColor("#e67e80"),
            "object": QColor("#a7c080"),
            "define_macro": QColor("#596879"),
            "dunder_method": QColor("#dbbc7f"),  # Золотистый, хорошо сочетается с Everforest
            "caret": QColor("#d3c6aa"),
            "caret_line": QColor("#2d3c4d"),
            "margin_bg": QColor("#2d3c4d"),
            "margin_fg": QColor("#859289"),
            "selection_bg": QColor("#4a5a6a"),
            "selection_fg": QColor("#d3c6aa"),
            "brace_bg": QColor("#e6986c55"),
            "brace_fg": QColor("#e6986c"),
            "error": QColor("#e67e80"),
            "warning": QColor("#e6986c"),
            "echo": QColor("#9eaf9a"),
            "autosave_on": QColor("#a7c080"),
            "autosave_off": QColor("#e67e80"),
            "status_bg": QColor("#1a2632"),
            "status_border": QColor("#4a5a6a"),
            "tree_hover_bg": QColor("#2f3f4f"),
            "tree_hover_fg": QColor("#d3c6aa"),
            "tree_selection_bg": QColor("#3a4a5a"),
            "tree_selection_fg": QColor("#d3c6aa"),
            "tree_edit_bg": QColor("#26323f"),
            "tree_edit_fg": QColor("#d3c6aa"),
        }
    },

    "Rosé Pine": {
    "type": "dark",
    "colors": {
        "bg": QColor("#191724"),
        "fg": QColor("#e0def4"),
        "title_bg": QColor("#26233a"),
        "title_bg_darker": QColor("#1c192d"),
        "title_fg": QColor("#e0def4"),
        "title_border": QColor("#403d52"),
        "keyword": QColor("#ebbcba"),
        "type": QColor("#9ccfd8"),
        "comment": QColor("#6e6a86"),
        "string": QColor("#f6c177"),
        "number": QColor("#c4a7e7"),
        "operator": QColor("#6e6a86"),
        "function": QColor("#ebbcba"),
        "modifier": QColor("#c4a7e7"),
        "directive": QColor("#eb6f92"),
        "literal": QColor("#f6c177"),
        "namespace": QColor("#9ccfd8"),
        "special": QColor("#eb6f92"),
        "object": QColor("#c4a7e7"),
        "define_macro": QColor("#454050"),
        "dunder_method": QColor("#c4a7e7"),  # Фиолетовый из палитры Rosé Pine
        "caret": QColor("#e0def4"),
        "caret_line": QColor("#26233a"),
        "margin_bg": QColor("#1f1d2e"),
        "margin_fg": QColor("#6e6a86"),
        "selection_bg": QColor("#403d52"),
        "selection_fg": QColor("#e0def4"),
        "brace_bg": QColor("#ebbcba55"),
        "brace_fg": QColor("#ebbcba"),
        "error": QColor("#eb6f92"),
        "warning": QColor("#f6c177"),
        "echo": QColor("#908caa"),
        "autosave_on": QColor("#9ccfd8"),
        "autosave_off": QColor("#eb6f92"),
        "status_bg": QColor("#100e1a"),
        "status_border": QColor("#403d52"),
        "tree_hover_bg": QColor("#1c192d"),
        "tree_hover_fg": QColor("#e0def4"),
        "tree_selection_bg": QColor("#26233a"),
        "tree_selection_fg": QColor("#e0def4"),
        "tree_edit_bg": QColor("#191724"),
        "tree_edit_fg": QColor("#e0def4"),
    }
},

    "Nord Aurora": {
        "type": "dark",
        "colors": {
            "bg": QColor("#141f2e"),
            "fg": QColor("#d8dee9"),
            "title_bg": QColor("#2e3b4e"),
            "title_bg_darker": QColor("#232e3f"),
            "title_fg": QColor("#e5e9f0"),
            "title_border": QColor("#3b4c63"),
            "keyword": QColor("#81a1c1"),
            "type": QColor("#8fbcbb"),
            "comment": QColor("#4c5b73"),
            "string": QColor("#a3be8c"),
            "number": QColor("#b48ead"),
            "operator": QColor("#616e88"),
            "function": QColor("#88c0d0"),
            "modifier": QColor("#b48ead"),
            "directive": QColor("#d08770"),
            "literal": QColor("#ebcb8b"),
            "namespace": QColor("#8fbcbb"),
            "special": QColor("#d08770"),
            "object": QColor("#a3be8c"),
            "define_macro": QColor("#465164"),
            "dunder_method": QColor("#b48ead"),  # Фиолетовый из палитры Nord
            "caret": QColor("#e5e9f0"),
            "caret_line": QColor("#2e3b4e"),
            "margin_bg": QColor("#1a2538"),
            "margin_fg": QColor("#4c5b73"),
            "selection_bg": QColor("#3b4c63"),
            "selection_fg": QColor("#e5e9f0"),
            "brace_bg": QColor("#81a1c155"),
            "brace_fg": QColor("#81a1c1"),
            "error": QColor("#bf616a"),
            "warning": QColor("#d08770"),
            "echo": QColor("#7a8ba5"),
            "autosave_on": QColor("#a3be8c"),
            "autosave_off": QColor("#bf616a"),
            "status_bg": QColor("#0d1520"),
            "status_border": QColor("#2e3b4e"),
            "tree_hover_bg": QColor("#232e3f"),
            "tree_hover_fg": QColor("#e5e9f0"),
            "tree_selection_bg": QColor("#2e3b4e"),
            "tree_selection_fg": QColor("#e5e9f0"),
            "tree_edit_bg": QColor("#141f2e"),
            "tree_edit_fg": QColor("#e5e9f0"),
        }
    },

    "Gruvbox Dark": {
        "type": "dark",
        "colors": {
            "bg": QColor("#1d2021"),
            "fg": QColor("#ebdbb2"),
            "title_bg": QColor("#3c3836"),
            "title_bg_darker": QColor("#2e2a29"),
            "title_fg": QColor("#fbf1c7"),
            "title_border": QColor("#504945"),
            "keyword": QColor("#fb4934"),
            "type": QColor("#8ec07c"),
            "comment": QColor("#928374"),
            "string": QColor("#b8bb26"),
            "number": QColor("#d3869b"),
            "operator": QColor("#fe8019"),
            "function": QColor("#fabd2f"),
            "modifier": QColor("#fe8019"),
            "directive": QColor("#d65d0e"),
            "literal": QColor("#d3869b"),
            "namespace": QColor("#83a598"),
            "special": QColor("#d65d0e"),
            "object": QColor("#8ec07c"),
            "define_macro": QColor("#544848"),
            "dunder_method": QColor("#d3869b"),  # Розовый из палитры Gruvbox
            "caret": QColor("#fbf1c7"),
            "caret_line": QColor("#3c3836"),
            "margin_bg": QColor("#282828"),
            "margin_fg": QColor("#928374"),
            "selection_bg": QColor("#504945"),
            "selection_fg": QColor("#fbf1c7"),
            "brace_bg": QColor("#fb493455"),
            "brace_fg": QColor("#fb4934"),
            "error": QColor("#cc241d"),
            "warning": QColor("#d79921"),
            "echo": QColor("#a89984"),
            "autosave_on": QColor("#b8bb26"),
            "autosave_off": QColor("#cc241d"),
            "status_bg": QColor("#141617"),
            "status_border": QColor("#3c3836"),
            "tree_hover_bg": QColor("#2e2a29"),
            "tree_hover_fg": QColor("#ebdbb2"),
            "tree_selection_bg": QColor("#3c3836"),
            "tree_selection_fg": QColor("#ebdbb2"),
            "tree_edit_bg": QColor("#1d2021"),
            "tree_edit_fg": QColor("#ebdbb2"),
        }
    },
    # Тема Monokai Pro (тёмная)
"Monokai Pro Dark": {
    "type": "dark",
    "colors": {
        "bg": QColor("#2d2a2e"),
        "fg": QColor("#fcfcfa"),
        "title_bg": QColor("#403e41"),
        "title_bg_darker": QColor("#353335"),
        "title_fg": QColor("#fcfcfa"),
        "title_border": QColor("#4e4b4f"),
        "keyword": QColor("#f92672"),
        "type": QColor("#66d9ef"),
        "comment": QColor("#75715e"),
        "string": QColor("#e6db74"),
        "number": QColor("#ae81ff"),
        "operator": QColor("#fcfcfa"),
        "function": QColor("#a6e22e"),
        "modifier": QColor("#fd971f"),
        "directive": QColor("#f92672"),
        "literal": QColor("#ae81ff"),
        "namespace": QColor("#66d9ef"),
        "special": QColor("#f92672"),
        "object": QColor("#a6e22e"),
        "define_macro": QColor("#6e6b6f"),
        "dunder_method": QColor("#a6e22e"),
        "caret": QColor("#fcfcfa"),
        "caret_line": QColor("#3e3a3f"),
        "margin_bg": QColor("#242125"),
        "margin_fg": QColor("#75715e"),
        "selection_bg": QColor("#49454a"),
        "selection_fg": QColor("#fcfcfa"),
        "brace_bg": QColor("#f9267255"),
        "brace_fg": QColor("#f92672"),
        "error": QColor("#f44747"),
        "warning": QColor("#e6db74"),
        "echo": QColor("#928f94"),
        "autosave_on": QColor("#a6e22e"),
        "autosave_off": QColor("#f44747"),
        "status_bg": QColor("#1a181c"),
        "status_border": QColor("#403e41"),
        "tree_hover_bg": QColor("#353335"),
        "tree_hover_fg": QColor("#fcfcfa"),
        "tree_selection_bg": QColor("#403e41"),
        "tree_selection_fg": QColor("#fcfcfa"),
        "tree_edit_bg": QColor("#2d2a2e"),
        "tree_edit_fg": QColor("#fcfcfa"),
    }
},

    "Emerald Forest": {
        "type": "dark",
        "colors": {
            "bg": QColor("#182018"),
            "fg": QColor("#cfddcf"),
            "title_bg": QColor("#2a302a"),
            "title_bg_darker": QColor("#212621"),
            "title_fg": QColor("#dfecdf"),
            "title_border": QColor("#3a403a"),
            "keyword": QColor("#c6a0f6"),
            "type": QColor("#7dc4e4"),
            "comment": QColor("#6a746a"),
            "string": QColor("#f5b88c"),
            "number": QColor("#e2bf7a"),
            "operator": QColor("#8a988a"),
            "function": QColor("#c8d96c"),
            "modifier": QColor("#b0a5f0"),
            "directive": QColor("#d99e6e"),
            "literal": QColor("#f5aa8c"),
            "namespace": QColor("#6cc8b4"),
            "special": QColor("#e68c8c"),
            "object": QColor("#9acc8c"),
            "define_macro": QColor("#4a504a"),
            "dunder_method": QColor("#c6a0f6"),  # Лавандовый из палитры Emerald Forest
            "caret": QColor("#f0f8f0"),
            "caret_line": QColor("#2a302a"),
            "margin_bg": QColor("#1e241e"),
            "margin_fg": QColor("#6a746a"),
            "selection_bg": QColor("#3a443a"),
            "selection_fg": QColor("#dfecdf"),
            "brace_bg": QColor("#c6a0f655"),
            "brace_fg": QColor("#c6a0f6"),
            "error": QColor("#e68c8c"),
            "warning": QColor("#e2bf7a"),
            "echo": QColor("#9aae9a"),
            "autosave_on": QColor("#c8d96c"),
            "autosave_off": QColor("#e68c8c"),
            "status_bg": QColor("#0f140f"),
            "status_border": QColor("#2a302a"),
            "tree_hover_bg": QColor("#212621"),
            "tree_hover_fg": QColor("#cfddcf"),
            "tree_selection_bg": QColor("#2a302a"),
            "tree_selection_fg": QColor("#cfddcf"),
            "tree_edit_bg": QColor("#182018"),
            "tree_edit_fg": QColor("#cfddcf"),
        }
    },
}
