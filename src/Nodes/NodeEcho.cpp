...
struct NodeEcho : public Node { NO_EVAL
    ...
    NodeEcho(Node* expr, Token start_token, Token end_token)
    ...
    void exec_from(Memory* _memory) override {
        ...
        if (value.type == STANDART_TYPE::TYPE)
            echo_message = any_cast<Type&>(value.data).pool;
        else if (value.type == STANDART_TYPE::STRING)
            echo_message = any_cast<string&>(value.data);
        else if (value.type == STANDART_TYPE::CHAR)
            echo_message = any_cast<char>(value.data);
            ...
            echo_message = to_string(any_cast<int64_t>(value.data));
        } 
        else if (value.type == STANDART_TYPE::DOUBLE) {
            echo_message = to_string(any_cast<NUMBER_ACCURACY>(value.data));
        }
        else if (value.type == STANDART_TYPE::NULL_T) {
            echo_message =  "null";
        }
        else if (value.type == STANDART_TYPE::NAMESPACE) {
            echo_message = "namespace " + any_cast<Namespace>(value.data).name;
        }
