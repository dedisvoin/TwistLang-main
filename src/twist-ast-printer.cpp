...
void debug_print_ast(const vector<std::unique_ptr<Node>>& modes, bool use_colors = true) {
    ASTPrinter printer;
    if (!use_colors) {
        printer.disable_colors();
    }
    printer.print_to_console(modes, "Twist Language AST");
}

inline void save_ast_to_file(const vector<std::unique_ptr<Node>>& nodes, const std::string& filename, bool use_colors = false) {
    ASTPrinter printer;
    printer.save_to_file(nodes, filename, use_colors);
}
